SELECT ROW_NUMBER() OVER(ORDER BY T1.BUILD_NO, T2.OFLEVEL_ORDER, CAST(substring(T3.UNIT_NO,charindex('/',T3.UNIT_NO)+(1),len(T3.UNIT_NO)) as numeric) ASC ) AS ROWNUM_, T4.USE_FLG, T1.CONDO_B_ID, T1.BUILD_NO, T1.BUILD_GROUP, T2.CONDO_L_ID, T3.CONDO_H_ID, T2.OFLEVEL AS OFLEVEL, T2.OFLEVEL_ORDER, T3.UNIT_NO, T3.ROOM_TYPE_ID, T4.DATE_PRICE AS DATE_PRICE, YEAR(T4.DATE_PRICE)+543 AS YEAR_PRICE , T4.AREA_CONTRACT AS AREA_CONTRACT, T4.PRICE_S, T4.PRICE_S_PERM, T4.PRICE_DISCOUNT, T4.PRICE_FUR, T4.PRICE_FUR_PERM, T4.SUM_PRICE_S, T4.SUM_PRICE_S_PERM, T4.PRICE_FUR_EDIT_AREA, T4.SUM_PRICE_FUR_EDIT, T4.PRICE_S_EDIT AS PRICE_S_EDIT, ROUND(T4.PRICE_S_EDIT_PERM,0) AS PRICE_S_EDIT_PERM
                    , CASE WHEN T4.DATE_PRICE IS NULL THEN '*' ELSE '' END START_PRICELIST
                    FROM(
                    ...
                    WHERE USE_FLG = 1
                    ORDER BY T1.BUILD_NO, T2.OFLEVEL_ORDER, CAST(substring(T3.UNIT_NO,charindex('/',T3.UNIT_NO)+(1),len(T3.UNIT_NO)) as numeric)