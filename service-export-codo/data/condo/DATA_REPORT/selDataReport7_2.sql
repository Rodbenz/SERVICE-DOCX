-- SELECT MIN_S_D_FUR_AR, MAX_S_D_FUR_AR, ROUND((SUM_S_D_FUR_AR / COUNT_S_D_FUR_AR), 0) SUM_COUNT, ROUND(CAST((SUM_S_D_FUR_AR / COUNT_S_D_FUR_AR) as float) / 100, 0) * 100 SUM_COUNT_PATSAT, MIN_DATE_PRICE, MAX_DATE_PRICE
-- FROM(
--   SELECT MIN(S_D_FUR_AR) MIN_S_D_FUR_AR, MAX(S_D_FUR_AR) MAX_S_D_FUR_AR, COUNT(S_D_FUR_AR) COUNT_S_D_FUR_AR, SUM(TTX1.S_D_FUR_AR) SUM_S_D_FUR_AR, MIN(DATE_PRICE) MIN_DATE_PRICE, MAX(DATE_PRICE) MAX_DATE_PRICE
--   FROM(
...
--         WHERE CONDO_S_ID = @CONDO_S_ID AND USE_FLG = 1
--         ) T1
--         LEFT OUTER JOIN
--         (
...
--   ) TTX1
-- ) TYY1
SELECT MIN_S_D_FUR_AR, MAX_S_D_FUR_AR, ROUND((SUM_S_D_FUR_AR / COUNT_S_D_FUR_AR), 0) SUM_COUNT, ROUND(CAST((SUM_S_D_FUR_AR / COUNT_S_D_FUR_AR) as float) / 100, 0) * 100 SUM_COUNT_PATSAT, MIN_DATE_PRICE, MAX_DATE_PRICE, TYY1.BUILD_NO
FROM(
  SELECT MIN(S_D_FUR_AR) MIN_S_D_FUR_AR, MAX(S_D_FUR_AR) MAX_S_D_FUR_AR, COUNT(S_D_FUR_AR) COUNT_S_D_FUR_AR, SUM(TTX1.S_D_FUR_AR) SUM_S_D_FUR_AR, MIN(DATE_PRICE) MIN_DATE_PRICE, MAX(DATE_PRICE) MAX_DATE_PRICE, TTX1.BUILD_NO
  FROM(
    SELECT TTA1.CONDO_B_ID, TTA1.BUILD_NO, TTA1.OFLEVEL_ORDER, CAST(ROUND((TTA1.S_D_FUR / TTA1.AREA_CONTRACT),0) AS int) AS S_D_FUR_AR, DATE_PRICE, ROOM_TYPE_ID
      FROM(
        SELECT T4.CONDO_B_ID, T4.BUILD_NO, T3.OFLEVEL_ORDER, (ISNULL(T1.PRICE_S,0) - ISNULL(T1.PRICE_DISCOUNT,0) - ISNULL(T1.PRICE_FUR_EDIT_AREA,0)) AS S_D_FUR, T1.AREA_CONTRACT, T1.DATE_PRICE, T2.ROOM_TYPE_ID
